...
const Navbar = () => {
    ...
    const handleSearch = (e: React.SubmitEvent) => {
        ...
        if(searchQuery.trim()){
            ...
            setSearchQuery("");
            // here encode uri component makes the url safe 
            // for ex- search = "red apples" since url doesnt support spaces it makes it safe by adding special characters such as %20, so with encodeUriComponent, search -> red%20apples
        }
                ...
                <div className="relative">
                    { user ? (
                        ...
                        <div className="flex-center gap-2">
                            ...
                            onClick={() => setUserMenuOpen(!userMenuOpen)}/> : <MenuIcon className="md:hidden" onClick={() => setUserMenuOpen(!userMenuOpen)}/>}
                        </div>
                    )}
                            ...
                            <div className="fixed inset-0 z-40" onClick={() => setUserMenuOpen(false)}/>
                                <div className="absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg  border border-app-border py-2 z-50 animate-fade-in">
                                    ...
                                    <div onClick={() => setUserMenuOpen(false)} >
                                        ...
                                        <Link to='/deals' className="dropdown-link md:hidden"> <ArrowUpRightIcon size={16}/>
                                            Deals
                                        </Link>

                                        {user?.isAdmin && <Link to='/admin/products' className="dropdown-link "> <Shield size={16} className="text-app-orange-dark"/>
