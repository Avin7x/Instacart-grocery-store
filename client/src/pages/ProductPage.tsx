...
const ProductPage = () => {
  ...
  useEffect(() => {
      ...
      }).then(({data}) => setRelatedProducts(data.products.filter((p: Product) => p.id !== id))).catch((error: any) => {toast.error(error?.response?.data?.message || error?.message); navigate('/products')}).finally(() => setLoading(false));
  },
  [id, navigate]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            {/* left side - image */}
            <div className="relative flex-center p-8 md:p12 min-h-[320px] md:min-h-[480px]">
              <img
                src={product.image}
                alt={product.name}
                className="w-auto object-contain max-h-[360px]"
              />
                ...
                <div className="flex items-center gap-2 mb-5">
                  <div className="flex items-center gap-0.5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <StarIcon
                        key={star}
                        className={`w-3 h-3 ${star <= Math.round(product.rating) ? "text-app-warning fill-app-warning" : "text-app-border"}`}
                      />
                    ))}
                  </div>
        ...
        {items.length > 0 && (
          <section className="mt-12 mb-44">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link
                to={`/products?category=${product.category}`}
                className="text-sm font-semibold text-app-orange hover:text-app-orange-dark transition-colors flex items-center gap-1"
              >
