...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    setProduct(product!)
    // here ! at the end of product means we are saying typescript that product will not be undefined 
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="relative flex-center p-8 md:p12 min-h-[320px] md:min-h-[480px]">
                <img src={product.image} alt={product.name} className="w-auto object-contain max-h-[360px]"/>
              ...
              {product.rating > 0 && (
                  <div className="flex items-center gap-2 mb-5">
                    <div className="flex items-center gap-0.5">
                      {[1,2,3,4,5].map((star) => (
                        <StarIcon key={star} className={`w-3 h-3 ${star <= Math.round(product.rating) ? "text-app-warning fill-app-warning": "text-app-border" }`}/>
                      ))}
                    </div>
