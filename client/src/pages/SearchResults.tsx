...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    api.get(`/products?search=${encodeURIComponent(query)}`).then(({data}) => setProducts(data.products)).catch((error: any)=> toast.error(error?.response?.data?.message || error?.message)).finally(() => setLoading(false));
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm  text-app-text-light mb-6 max-w-md mx-auto">
              we could'nt find any products matching "{query}". Try a different
              search term.{" "}
            </p>

            <Link
              to="/products"
              className="inline-flex px-5 py-2.5 bg-app-green text-white text-sm font-semibold rounded-lg"
            >
              Browse All Products
            </Link>
          </div>
