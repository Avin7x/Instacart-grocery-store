...
const getAuthHeaders = () => ({headers: {Authorization: `Bearer ${localStorage.getItem("delivery_token")}`}});
...
export default function DeliveryDashboard() {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${apiUrl}/delivery/my-deliveries?status=${tab}`, getAuthHeaders());
  ...
  // send location every 10 seconds for active deliveries
  useEffect(() => {
    ...
    const sendLocation = (pos: GeolocationPosition) => {
      ...
      activeOrders.forEach(o => {
        axios.put(`${apiUrl}/delivery/my-deliveries/${o.id}/location`, {lat, lng}, getAuthHeaders()).catch(() =>{});
      })
    }
    ...
    // also send on interval for consistent updates
  ...
  const handleUpdateStatus = async (orderId: string, status: string) => {
    try {
      await axios.put(`${apiUrl}/delivery/my-deliveries/${orderId}/status`, {status}, getAuthHeaders());
  ...
  const handleComplete = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.put(`${apiUrl}/delivery/my-deliveries/${otpModal}/complete`, {otp}, getAuthHeaders());
  ...
  const handleCancel = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.put(`${apiUrl}/delivery/my-deliveries/${cancelModal}/cancel`, {reason: cancelReason}, getAuthHeaders());
