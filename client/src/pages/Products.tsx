...
const Products = () => {
  ...
  const clearQueryParam = () => setSearcParams({});
  ...
  return (
    <div className="min-h-screen bg-app-cream">
        ...
        <div className="flex gap-8 xl:gap-10">
          ...
          <aside className="hidden lg:block w-64 shrink-0">
            <div className="bg-white rounded-2xl p-4 sticky top-24">
              ...
              <FilterPanel categories={categoriesData} category={category} organic={organic} minPrice={minPrice} maxPrice={maxPrice} updateQueryParam={updateQueryParam} clearQueryParam={clearQueryParam} hasFilters={hasFilters}/>
            </div>
          </aside>
          ...
          <main className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex flex-col lg:items-center gap-3">
                ...
                <div className="relative">
                  <select value={sort} onChange={(e) => updateQueryParam("sort", e.target.value)} 
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <button onClick={() => clearQueryParam()} className="px-5 py-2 text-sm bg-app-green text-white rounded-xl hover:bg-app-green-light transition-colors">CLear Filters</button>
              </div>
            ) : (
              <div className="grid grid-cold-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 xl:gap-8">
            ...
            {totalPages > 1 && (
              <div className="flex-center gap-2 mt-16">
                {Array.from({length: totalPages}).map((_, i) => (
                  <button key={i} onClick={() =>{ updateQueryParam("page", `${i+1}`); scrollTo(0,0);}} className={`size-9 rounded-lg text-sm font-medium transition-colors ${page === i+1 ? "bg-app-green text-white" : "bg-white text-app-text-light hover:bg-app-cream"}`}>
                    {i+1}
                    </button>
                ))}
              </div>
      ...
      {mobileFiltersOpen && (
        <>
           <div onClick={() => setMobileFiltersOpen(false)} className="fixed inset-0 bg-black/40 z-50" />
